...
<head>
    ...
    <title>Edit Produk - Inventory Manager</title>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('products.index') }}" class="text-indigo-600 hover:text-indigo-800 transition-colors">
                    <i class="fas fa-arrow-left text-xl"></i>
                </a>
                <h1 class="text-4xl font-bold text-gray-900">
                    <i class="fas fa-edit text-indigo-600 mr-3"></i>Edit Produk
                </h1>
            </div>
            <p class="text-gray-600 ml-12">Perbarui informasi produk di bawah ini</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6">
                <h2 class="text-2xl font-bold text-white">
                    <i class="fas fa-pencil-alt mr-2"></i>Perbarui Informasi Produk
                </h2>
            </div>

            <!-- Form Content -->
            <form action="{{route('products.update', $products->id)}}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-4 pt-6 border-t border-gray-200">
                    ...
                    >
                        <i class="fas fa-check-circle"></i>Perbarui Produk
                    </button>
                    <button 
                        type="reset"
                    ...
                    <a 
                        ...
                        class="flex-1 bg-indigo-100 hover:bg-indigo-200 text-indigo-800 font-bold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2"
                    >
        ...
        <div class="mt-8 bg-blue-50 border-l-4 border-blue-500 rounded-lg p-4 shadow-md">
            <p class="text-sm text-blue-800">
                ...
                <strong>Tips:</strong> Pastikan semua perubahan telah sesuai sebelum menyimpan. Data produk akan segera diperbarui.
            </p>
        </div>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <p class="text-2xl font-bold text-gray-800">#{{ $products->id }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
                <p class="text-sm text-gray-600">Dibuat Pada</p>
                <p class="text-lg font-semibold text-gray-800">{{ $products->created_at->format('d M Y') }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
                <p class="text-sm text-gray-600">Diperbarui Pada</p>
                <p class="text-lg font-semibold text-gray-800">{{ $products->updated_at->format('d M Y H:i') }}</p>
            </div>
        </div>
    </div>
